...
$cleared = mysqli_fetch_assoc(mysqli_query(
    $conn,
    "SELECT COUNT(*) c FROM dues WHERE accounts_due = 0"
))['c'];
...
<head>
    ...
    <title>Accounts Dashboard | SCOE Digital System</title>
    ...
    <style>
        * {
        ...
        /* ----- COLOR SYSTEM (identical to student dashboard) ----- */
        :root {
            --primary: #1e293b;
        ...
        /* ----- CARDS GRID (same style as student dashboard) ----- */
        .grid {
            display: grid;
        ...
        /* ----- TABLE CARD (border-only, matches index.php table style) ----- */
        .table-card {
            background: var(--card-bg);
            border: 1px solid var(--border);
            border-radius: 12px;
...
<!-- ========== NAVBAR (EXACT SAME AS STUDENT DASHBOARD) ========== -->
<div class="navbar">
    <div class="logo">
    ...
    <div class="nav-links" id="navMenu">
       <a href="../index.php">Home</a>
        <a href="dashboards/accounts_dashboard.php" class="active">Dashboard</a>
...
<div class="container">
    ...
    <div class="grid">
        ...
        <div class="card">
            <div class="card-label">Cleared (Accounts)</div>
        ...
        <table>
            ...
            <?php if(mysqli_num_rows($result) > 0){ ?>
                ...
                <tr>
                    <td><?= $row['name'] ?></td>
                    <td><?= $row['prn'] ?></td>
                    <td>₹<?= number_format($row['accounts_due']) ?></td>

                  <td>
    <?php if($row['accounts_due'] > 0){ ?>
        <a href="delete_due.php?id=<?= $row['student_id'] ?>&type=accounts" class="btn btn-pending">
            Pending
        </a>
    <?php } else { ?>
    ...
    <?php } ?>
            ...
            <?php } else { ?>
                ...
                <tr class="empty-row">
                    <td colspan="4">No pending dues found</td>
                </tr>
