...
$cleared = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) c FROM dues WHERE exam_due=0"))['c'];
$pending = mysqli_fetch_assoc(mysqli_query($conn,"SELECT COUNT(*) c FROM dues WHERE exam_due>0"))['c'];
...
// ✅ FIXED QUERY
$query = "SELECT students.name, students.prn, dues.exam_due, students.student_id
          FROM dues
          JOIN students ON students.student_id = dues.student_id
          WHERE dues.exam_due > 0
          AND (
...
<head>
    ...
    <title>Student Section Dashboard | SCOE Digital System</title>
    ...
    <style>
        * {
        ...
        /* ----- NAVBAR (exactly as index.php & accounts) ----- */
        .navbar {
            display: flex;
        ...
        /* ----- CARDS GRID (same style as accounts dashboard) ----- */
        .grid {
            display: grid;
        ...
        /* ----- TABLE CARD (border-only, matches accounts dashboard) ----- */
        .table-card {
            background: var(--card-bg);
            border: 1px solid var(--border);
            border-radius: 12px;
        ...
        /* ----- FOOTER (identical to index.php & accounts) ----- */
        .footer {
            background: #111827;
            color: #9ca3af;
...
<div class="navbar">
    ...
    <div class="nav-links" id="navMenu">
        ...
        <a href="exam_dashboard.php" class="active">Dashboard</a>
...
<div class="container">
    ...
    <div class="dashboard-header">
        <div class="page-title">Student Section Dashboard</div>
    ...
    <!-- TABLE: pending exam dues (exam_due > 0) -->
    <div class="table-card">
        <h3> Pending Student Section Dues</h3>
        <table>
            <thead>
                <tr>
                    <th>Name</th>
                    <th>PRN</th>
                    <th>Student Section Due (₹)</th>
                    <th>Status</th>
                </tr>
            </thead>
            <tbody>
                <?php if (mysqli_num_rows($result) > 0): ?>
                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                        <tr>
                            ...
                            <td>₹<?= number_format($row['exam_due']) ?></td>
                            <td>
                                <?php if ($row['exam_due'] > 0): ?>
                                    <a href="delete_due.php?id=<?= $row['student_id'] ?>&type=exam" class="btn btn-pending">Pending</a>
                                <?php else: ?>
                ...
                <?php else: ?>
                    <tr class="empty-row">
                        <td colspan="4">No pending student section dues found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<!-- ========== FOOTER (EXACT MATCH FROM ACCOUNTS DASHBOARD) ========== -->
<footer class="footer">
...
<!-- mobile menu toggle (same as index & accounts) -->
<script>
